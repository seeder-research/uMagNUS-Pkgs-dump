...
func Write(out io.Writer, s *data.Slice, info data.Meta) error {
	...
	w.writeUInt64(data.SIZEOF_DATATYPE) // precision
		...
		for iz := 0; iz < size[2]; iz++ {
			for iy := 0; iy < size[1]; iy++ {
				for ix := 0; ix < size[0]; ix++ {
					if (data.SIZEOF_DATATYPE == 8) {
						w.writeFloat64(float64(dataArr[c][iz][iy][ix]))
					} else {
						w.writeFloat32(float32(dataArr[c][iz][iy][ix]))
					}
				}
			}
		}
